...
class visualizationWindow(Screen):
    ...
    def binder2(self,checkbox,value):
        if value:
            print(self.obj_dict[checkbox]," added")
        else:
            print(self.obj_dict[checkbox], " removed")
    def uploader(self, dt):
        ...
        for file in self.files:
            ...
            box=BoxLayout(size_hint_y=None,height=50,padding=[10,0,0,0])
            ...
            label=Label(text=f"[color=#ffffff]{file_name}[/color]",markup=True)
        ...
        for column in columns:
            box = BoxLayout(size_hint_y=None, height=50, padding=[10, 0, 0, 0])
            checkbox2=CheckBox(size_hint_x=.25)
            checkbox2.bind(active=self.binder2)
            label = Label(text=f"[color=#ffffff]{column}[/color]", markup=True)
            box.add_widget(checkbox2)
            box.add_widget(label)
            self.ids.property_placeholder.add_widget(box)
            self.column_dict[checkbox2]=column
    ...
    def update(self):
        ...
        # Create a FigureCanvas to display the graph
        canvas = FigureCanvas(figure=fig)
        self.ids.graph_placeholder.add_widget(canvas)
